...
def inference(mode, st_path, prompt_list, out_path = "result", num_steps = 20, scale = 10, size = 512):
    ...
    for prompt in prompt_list:
        image = main_pipeline(num_inference_steps=num_steps,
                              guidance_scale=scale,
                              width=size,
                              height=size,
                              prompt=prompt).images[0]
        image.save(out_path + "/" + prompt + ".png")